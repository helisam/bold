...
public class GlobalExceptionHandlerTest {
    ...
    @Test
    void handleCommunicationException_DeveRetornarStatus503() {
        // Arrange
        CommunicationException exception = new CommunicationException("Erro de comunicação");

        // Act
        ResponseEntity<ErrorResponse> responseEntity = exceptionHandler.handleCommunicationException(exception, webRequest);
        ...
        assertEquals("Erro de comunicação", responseEntity.getBody().getError());
        assertEquals("Erro de comunicação", responseEntity.getBody().getMessage());
    }
}